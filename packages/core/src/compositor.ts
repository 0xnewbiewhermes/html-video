...
export async function applyTextOverlay(args: {
  ...
  // Build ffmpeg drawtext filter for each overlay
  const drawtextFilters: string[] = [];
  for (let i = 0; i < overlays.length; i++) {
    const o = overlays[i]!;
    const text = String(variables[o.variable] ?? '').replace(/'/g, "'\\\\''");
    if (!text) continue; // skip empty variables

    // Position calculation: relative (0-1) → absolute pixels
    // ffmpeg drawtext uses (x, y) in pixels. We use 1920×1080 as base.
    ...
    // Escape text for ffmpeg filter graph:
    //   ' → '\\'' (single quotes are special in ffmpeg filter strings)
    //   \n → \\\\\\n (literal newline in drawtext)
    const safeText = text
      .replace(/'/g, "'\\\\\\''")
      .replace(/\\n/g, '\\\\\\n');

    const filter = `drawtext=text='${safeText}':fontfile=${o.fontFile}:fontsize=${o.fontSize}:fontcolor=${o.fontColor}:x=${px}:y=${py}:box=1:boxcolor=black@0.3:boxborderw=6`;
